...
void nRfInitProm(  uint8_t addrLength ){	//Init for Promiscuous reception 
    uint8_t aw_value;                       //(Everything which matches the addrLength MSBs of the 5 byte address)
    switch (addrLength){
        case 3:
            aw_value = 0b00000001;
        break;
        ...
        break;
        default:
        printf("error: addrLength must be 3, 4 or 5!\n");
        return;
    }
    initSPI();                              
	NRF_CE_OFF();
...
void nRfSendBytes( uint8_t *bytesToSend, uint8_t len, uint8_t *adr, uint8_t noAck ){
	...
	while( nTries < 10 ) {
	    ...
	    if( tempStatus & (1<<TX_DS) ){
	    	printf( " TX_ACK[%d,%d] ", nTries, nRfGetRetransmits() );
    	    nRfWrite_register( STATUS, (1<<TX_DS) );    //Clear Data ready flag
	        break;					//Done transmitting, break the for loop
	    }
	    if( tempStatus & (1<<MAX_RT) ){
	    	printf( " MAX_RT[%d] ", nTries++ );
    	    nRfWrite_register( STATUS, (1<<MAX_RT) );   //Clear MAX_RT flag
            // Data is still in TX-FIFO, try another time by toggling the CE pin
        	NRF_CE_OFF();
		    _del_ms( 1 );
        	NRF_CE_ON();
	    }
